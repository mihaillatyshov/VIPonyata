...
def get_lesson_activities(lesson_id: int):
    ...
    if drilling := DBQS.DrillingQueries.get_by_lesson_id(lesson_id, get_current_user_id()):
        drilling.tries = DBQS.DrillingQueries.get_tries_by_activity_id(drilling.id, get_current_user_id())

    if assessment := DBQS.AssessmentQueries.get_by_lesson_id(lesson_id, get_current_user_id()):
        assessment.tries = DBQS.AssessmentQueries.get_tries_by_activity_id(assessment.id, get_current_user_id())

    if hieroglyph := DBQS.HieroglyphQueries.get_by_lesson_id(lesson_id, get_current_user_id()):
        hieroglyph.tries = DBQS.HieroglyphQueries.get_tries_by_activity_id(hieroglyph.id, get_current_user_id())

    if final_boss := DBQS.FinalBossQueries.get_by_lesson_id(lesson_id, get_current_user_id()):
        final_boss.tries = DBQS.FinalBossQueries.get_tries_by_activity_id(final_boss.id, get_current_user_id())

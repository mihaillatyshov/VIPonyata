...
class DataBase:
    ...
    def AddTableElement(self, tableName, data):
        columns = ",".join(
            list(map(lambda key: "`" + str(key) + "`", data.keys())))
        values = ",".join(
            list(map(lambda key: "'" + str(data[key]) + "'", data.keys())))
        self.ExecuteQuery(
            f"INSERT INTO `{tableName}` ( {columns} ) VALUES ( {values} )")

    def UpdateTableElement(self, tableName, data, where=None):
        update_elements = ",".join(list(
            map(lambda key: "`" + str(key) + "` = '" + str(data[key]) + "'", data.keys())))
        update_elements = f"UPDATE {tableName} SET " + \
            update_elements + (f"WHERE {where} " if where != None else "")
    ...
    def GetTableElements(self, tableName, where=None, start=-1, count=65536, tableElements=None):
        ...
        select_elements = "SELECT " + elementsStr + \
            f" FROM {tableName} " + whereStr + limitStr
    ...
    def FieldToJsonFormat(self, field):
        if isinstance(field, (datetime, date, time, timedelta)):
            return str(field)
        return field
    ...
    def GetTableJson(self, tableName, where=None, start=-1, count=65536, tableElements=None):
        DBData = self.GetTableElements(
            tableName, where=where, start=start, count=count, tableElements=tableElements)
        DBColData = []
    ...
    def GetTablesJson(self, tables, where=None, start=-1, count=65536):
        ...
        for name, info in tables.items():
            ...
            if info.get("elements"):
                # Select all elements form table
                if info["elements"] == self.MacroAllTableElements():
                    columns = self.GetTableElementsNames(name)
                    for column in columns:
                        columnArr.append(column[0])
                    selectArr.append(f"{name}.*")
                # Select some elements form table
                elif type(info["elements"]) is list:
        ...
        data = self.GetTableElements(
            fromStr, where=where, start=start, count=count, tableElements=selectStr)

...
export default function useTimer({ deadline, interval = SECOND / 2, onDeadline = undefined }) {
	...
	const getDays    = () => { return Math.floor( timespan / DAY) }
	const getHours   = () => { return Math.floor((timespan / HOUR) % 24) }
	const getMinutes = () => { return Math.floor((timespan / MINUTE) % 60) }
	const getSeconds = () => { return Math.floor((timespan / SECOND) % 60) }
	...
	return {
		...
		minutes: getMinutes(),
		seconds: getSeconds(),
		getStrHHMMSS: () => { return getVV(getHours()) + ":" + getVV(getMinutes()) + ":" + getVV(getSeconds()) }
	};
}
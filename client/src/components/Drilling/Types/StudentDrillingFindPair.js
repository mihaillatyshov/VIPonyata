...
const StudentDrillingFindPair = ({ pairs }) => {
	...
	useEffect(() => {
		...
		if (pairs) {
			LogInfo(pairs)
			dispatch(setDrillingSelectedItem({ ...pairs, type: taskTypeName, selectedField : { "id" : -1, type : "None" }, doneFields : [] }))
		}
	}, [])
	...
	const getCardClassName = (id, type) => {
		if (isInDoneFields(id, type))
			return "col-auto findPairCorrect"
		if (item.selectedField.type === type && item.selectedField.id === id)
			return "col-auto findPairSelected"
									...
									<div className="row justify-content-end">
										{
											item.WordsJP.map((value, key) => (
												<Card className={getCardClassName(key, strWordsJP)} key={key} onClick={() => selectField(key, strWordsJP)}>
													{value}
												</Card>
											))
										}
									...
									<div className="row">
										{
											item.WordsRU.map((value, key) => (
												<Card className={getCardClassName(key, strWordsRU)} key={key} onClick={() => selectField(key, strWordsRU)}>
													{value}
												</Card>
											))
										}
